<template>
  <div class="cinema_body">
    <ul>
      <li v-for="item in cinemaList" :key="item_id">
        <div>
          <span>{{item.nm}}</span>
...
<!--          <div>{{item.tag.vipTag}}</div>-->
          <div v-for="(num,key) in item.tag" v-if="num===1" :class="key | classCard" :key="itemCard">{{key | formatCard}}</div>
        </div>
      </li>
    </ul>
  </div>
    ...
    filters:{
      formatCard(key){
        ...
        for(var i=0;i<card.length;i++){
          if(card[i].key===key) return card[i].value;
        }
        return '';
      },
      classCard(key){
